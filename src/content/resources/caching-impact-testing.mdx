...
**Cold cache test** — start with an empty cache. Run your load test. This shows worst-case performance and measures how quickly the cache warms up. See [cold start performance](/resources/cold-start-performance) for more on this pattern.
...
**Resource usage** — does caching reduce database load? CPU usage? This matters for capacity planning. If your database is the bottleneck, the [database bottlenecks](/resources/database-bottlenecks) guide can help.
...
Sometimes caching is a 10x win. Sometimes it's marginal. You won't know until you test. For broader optimization strategies, see [response time optimization](/resources/response-time-optimization).
